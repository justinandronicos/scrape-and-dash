...
from typing import Dict, Tuple, Set, Iterator, BinaryIO
...
def wm_file_parser() -> Iterator[Tuple[str, ProductItem]]:
    # Tuple[Set[str], Iterator[ProductItem]]:
    """Parses CSV of WM products

    Args: None

    Returns:
        Tuple(Set, Dict): Tuple contains (Set of brands,
        nested Dictionary of form {code: {brand, product_name, variant, retail_price, on_sale, current_price, in_stock, product_url}})
    """
    # brands_set: Set[str] = set()
    # products_dict: Dict[str, Dict[str, Union[str, Decimal, bool]]] = {}
    with open(cfg["wm_product_file"]) as f:
        ...
        for product_row in reader:
            ...
            try:
                ...
                print(product_name, product_row[3], product_row[4])
...
def wm_brands_pipeline(brand: str) -> None:
    ...
    existing_brand = session.query(brand_table).filter_by(name=brand_obj.name).first()
...
def wm_products_pipeline(product: ProductItem) -> None:
    """Save WM Products in the database

    Args:
        wm_products (Dict): Nested dict of wm products parsed from product csv
        of form {code: {brand, product_name, variant, retail_price, on_sale, current_price, in_stock, product_url}})
    """
    logging.basicConfig(
    ...
    # for code, product_dict in product.items():
    product_obj = WMProduct()
    product_obj.code = product["id"]
    ...
    if existing_product is not None:  # the current product exists
        logging.log(
            logging.INFO,
            f"Duplicate product item found: {product['product_name']}",
        )
        session.close()
...
def main():
    ...
    for brand, product in wm_file_parser():
        if brand not in brands:
            wm_brands_pipeline(brand)
            brands.add(brand)
        wm_products_pipeline(product)
